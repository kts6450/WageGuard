...
**WageGuard — 공공 마이크로데이터·AI 기반 이주노동자 임금 착취 위험 스크리닝·감독 우선순위 제안 시스템**
...
| 핵심 가치 | E9 근로자는 표준근로계약을 체결하고도 **언어 장벽·체류자격 종속**으로 위반에 대응·신고하지 못하는 구조 — 신고할 수 없는 피해자를 국가가 **먼저 찾아가도록** 점검 우선순위를 데이터로 제안 |
...
| 실증 상태 | **전면 공개 데이터로 재구축·검증 완료** — 국민연금 사업장 54만 곳 실측 위험 지표 + 임금체불 공개 명단 769명(중복 제거) 외부 채점 / 전 구간 작동 대시보드 |
| 외부 검증(실측) | 위험 상위 10% 셀(고용 6%)이 체불 사업주 **30% 포착 — 고용 기준 5.2배·사업장 수 기준 2.4배**, 홀드아웃 차수(2025~26)만 채점해도 **4.7배** / 순열검정 p=0.006 / 건설업 제외에도 4.0배 / 가중치 섭동 순위 상관 0.93 |
...
- **E9(비전문취업) 근로자는 고용허가제상 표준근로계약서를 체결하고 입국하지만**, 언어 장벽과 체류자격(비자) 종속 때문에 계약과 다른 실제 근로조건(임금 삭감·과도한 공제·초과근무 미지급)에 **대응하거나 신고하기 어렵다**. 계약의 존재가 곧 권리 행사로 이어지지 않는 것이다. 이것이 이주노동자 임금 착취 **신고율이 5% 미만**에 그치는 구조적 이유다.
- 국내 외국인 취업자는 **110만 9천 명**(통계청 2025년 이민자체류실태 및 고용조사, 2025.5월 기준)으로 역대 최고이며 전년 대비 9.8% 증가했다. 광·제조업 46만 1천 명, E-9 비전문취업 32만 1천 명 등 제조·농축산·건설·서비스 영역에 집중되어 있다. 임금 체불·삭감·과도한 공제·초과근무 미지급 등 착취형 위반은 광범위하게 발생하지만 피해자가 신고에 도달하지 못하는 이유는 다음과 같다.
...
본 아이디어 **WageGuard**는 고용노동부·한국고용정보원의 공공데이터와 통계청 마이크로데이터를 입력으로 받는 **AI 위험 스코어링**을 통해, 신고 이전에도 **산업×지역×고용형태 셀 단위의 위반 밀집도를 프로파일링해 감독 우선순위**를 도출한다. 표본조사의 특성상 **개별 사업장을 지목하지 않으며**, 실제 점검 대상 선정은 감독관이 행정데이터로 판단한다. 결과는 자동 처벌이 아닌 **근로감독·지원 자원의 배분 보조**로 사용된다.
...
| **이민자체류실태 및 고용조사 — 비전문취업(E9) 부가항목** | 통계청 | 약 1.1만 건 | **계약–현실 근로조건 괴리·이직 패턴** → 이주노동자 위험 보정 |
...
- 본 저장소의 **합성 시계열 시뮬레이션**(실측 분포로 보정한 모의 데이터)에서 3개월 시퀀스의 조기 탐지 AUROC가 LNN 0.9957 / LSTM 0.9900, 수렴 속도는 LNN이 LSTM 대비 약 25% 빠름으로 확인되었다. MDIS는 단면조사라 실제 임금 지급 시계열이 없으므로, **실데이터 기반 시계열 검증은 향후 과제**다(§7 로드맵 1단계).
...
(a)는 실제 데이터 5-Fold 학습(n=240,204)에서 도출된 전역 변수 중요도다. 사회보험 점수가 47.6%로 압도적인데, 검토 결과 이 변수는 라벨 정의(4대보험 미가입)와 정보가 겹치는 **파생 변수 누수**로 확인되었다. 이 진단에 따라 해당 변수와 시간당임금을 피처에서 제거하는 재학습을 설계했고, 초기 판별 성능 수치는 성과 지표에서 제외했다. 즉 설명 가능성은 (b)(c)(d)처럼 위험도가 다른 표본 레코드(H 0.94 / M 0.55 / L 0.10)의 변수별 +/− 기여를 SHAP 방식으로 분해해 감독관의 판단을 보조하는 도구인 동시에, 이번 사례처럼 **모델 자체의 결함(누수)을 드러내는 감사 도구**로도 작동한다.
...
(a)는 정상 사업장(30일 주기·정액 지급)과 의심 사업장(25~60일 불규칙 간격·금액 변동)의 임금 지급 시계열이다. (b)는 동일 케이스에서 LNN과 LSTM의 경보 시점 차이를 보여주는데, LNN은 6개월차에, LSTM은 10개월차에 경보를 띄웠다. (c)에서 1,000건의 **합성 시계열 시뮬레이션**(실측 분포로 보정한 모의 데이터)을 누적해 보면 LNN이 LSTM보다 평균 약 23일 일찍 위반 신호를 잡는다. 이 수치는 합성 데이터 기준이며, 실데이터 시계열 검증은 향후 과제다.
...
1. **기존 부정수급 탐지 시스템과의 근본적 차이**  
   - 고용보험 부정수급 AI 등 기존 시스템은 **사업주가 신고·신청한 행정 데이터의 모순**을 탐지한다. WageGuard는 **신고 자체가 발생하지 않는 임금·근로 마이크로데이터의 이상 패턴**에서 위험을 찾는다. 탐지 대상(착취 의심 vs. 부정수급)과 데이터 계층(미시 근로실태 vs. 행정 신고)이 근본적으로 다르다.
   - 고용노동부·고용정보원 공식 개방 데이터 + 통계청 MDIS를 결합하여 외부 그라운드 트루스(임금체불 명단·근로사건) 검증까지 한 번에 수행한 구조는 국내 선례가 없다.
...
| 이주노동자 보호 | 사후 구제 중심 | **고위험 셀(산업×지역) 사전 안내·통역 지원** |
...
| XGBoost 판별 성능 | 초기 0.9998 → **성과 지표에서 제외** | 라벨 파생 변수(사회보험 점수 등) 누수를 자체 확인 — 누수 제거 재학습 후 정직한 수치로 재보고 |
| LNN 3개월 조기 AUROC | **0.9957** (LSTM 0.9900) · **합성 시계열 기준** | 실데이터 시계열 검증은 로드맵 1단계 과제 |
| LNN 수렴 속도 | LSTM 대비 **약 25% 빠름** (동일 합성 실험) | 운영·재학습 비용 절감 |
| 이주노동자 미신고 추정 | **신고율 5% 미만** | 언어·체류자격 종속에 따른 신고 도달 실패 (신고 의존 한계의 구조적 근거) |
| **외부 검증 (실측 완료)** | 국민연금 실측 위험 지표(체불 정보 미사용)가 임금체불 공개 명단 769명(중복 제거, 매칭 709명)을 채점 — 상위 10% 셀 포착률 30%: **고용 기준 5.2배 / 사업장 수 기준 2.4배**, 지표 산출에 쓰지 않은 홀드아웃 차수(2025~26)만 27%(**4.7배**) | **순환논리가 구조적으로 불가능한 검증** — 지표와 채점 데이터의 소스가 독립 |
| 강건성 | 순열검정(무작위 1만 회) **p=0.006** · 건설업 전체 제외에도 4.0배(고용)/2.9배(사업장) · 무작위 가중 200조합 섭동에도 순위 상관 **평균 0.93** | 우연·업종 편중·임의 가중치 논란 정량 반박 |

### 6-3. 점검 효율 환산 — 같은 인력으로 얼마나 더 적발하나

이 절의 수치는 가정이 아니라 **실측**이다. 국민연금 행정데이터만으로 산출한 셀 위험 지표(체불 정보 미사용)에 따라 위험 상위 셀부터 점검한다고 할 때, 실제 임금체불 사업주 공개 명단(2023~2026, 중복 제거 769명 중 709명 매칭)이 얼마나 포착되는지를 직접 채점했다. 효율은 기준을 두 가지로 병기한다 — 근로자 보호 커버리지 관점의 **고용(가입자) 기준**과, 점검 행정 단위인 **사업장 수 기준**.

| 점검 범위 (위험 상위 셀) | 포착률(실측) | 고용 기준 효율 | 사업장 수 기준 효율 |
|--------------|------------|------------|------|
| 상위 10% 셀 (21개) | **29.6%** | **5.2배** (고용 5.7%) | **2.4배** (사업장 12.3%) |
| 상위 20% 셀 (41개) | **38.5%** | **3.8배** (고용 10.1%) | **2.1배** (사업장 18.7%) |
| 상위 10% 셀 — 홀드아웃 차수(2025~26)만 | 26.6% | **4.7배** | — |

> 고용의 6%에 해당하는 구역만 점검해도 실제 체불 사업주의 30%가 그 안에서 나온다. 지표 산출에 전혀 사용하지 않은 **홀드아웃 차수(2025~2026년 공개분) 명단에서도 4.7배가 유지**된다. 강건성 검증: 무작위 순위 1만 회 순열검정에서 이 포착률이 우연히 나올 확률은 **p=0.006**이며, 체불이 가장 많은 건설업 셀을 전부 제외해도 고용 기준 4.0배·사업장 기준 2.9배가 유지된다 — 특정 업종 편중이 아니라 저임금·고이직·영세성이라는 구조 신호다.
...
본 아이디어 **WageGuard**는 ① 고용노동부·한국고용정보원의 **공식 개방 공공데이터**를 핵심으로 사용하고, ② 통계청 MDIS 마이크로데이터로 **법정 기준 라벨**을 정밀화하며, ③ XGBoost·LNN·PyOD를 결합한 **AI 위험 스코어링**과 **외부 그라운드 트루스 검증**(임금체불 명단·근로사건)을 통해, 신고 이전 단계에서 **이주노동자 임금 착취 위험이 밀집한 산업×지역×고용형태 셀을 선제 스크리닝**한다. 결과는 자동 처벌이 아닌 **근로감독·지원 자원의 우선순위 제안**으로 환원되어, 같은 행정 자원으로 더 많은 위반을 적발·시정하고 더 많은 이주노동자를 보호할 수 있다.

이 출품작은 아이디어 부문으로 제출되었지만, 통상 PoC 단계의 산출물(491만 건 분석 데이터, 전 구간 작동 파이프라인, 작동하는 Streamlit 대시보드, 공개 저장소)을 이미 갖췄다. 초기 판별 성능 수치는 라벨 파생 변수 누수를 스스로 확인해 성과에서 제외했고, 누수 제거 재학습과 외부 데이터 검증을 설계해 두었다 — 화려한 수치보다 검증 가능한 정직함을 앞세우는 것이 정책 도구의 전제라고 보기 때문이다. 수상 후 별도 개발 기간 없이 정책 검증과 시범 적용에 곧바로 착수할 수 있으며, 고용노동부의 정책 방향인 노동 약자 보호·미조직 근로자 지원·찾아가는 행정을 데이터와 AI 인프라로 구현하는 한 가지 구체적 방식이다.

**방법론 자가 검증과 한계 (선제 공개)**

- **순환논리 — 구조적으로 차단**: 위험 지표는 국민연금 행정데이터(저임금·고이직·영세성)만으로 산출하고, 임금체불 공개 명단은 채점에만 사용한다. 서로 독립된 기관의 데이터이므로 '답을 보고 답을 맞히는' 구조가 성립할 수 없으며, 미래 차수 명단에서도 4.7배가 유지됨을 확인했다.
- **누수 자체 확인(초기 PoC)**: MDIS 기반 초기 모델에서 사회보험 점수(중요도 47.6%)가 라벨 정보를 재주입함을 확인, 초기 AUROC 0.9998을 성과 지표에서 자진 제외했다. 재구축 검증은 외부 명단 포착률이라는 누수 불가능 지표로 교체했다.
- **가중치 임의성 — 정량 반박**: 동일가중 z-합성을 무작위 가중 200조합으로 섭동해도 셀 순위 상관 평균 0.93으로 유지되며, 지도 모델의 가중치는 과거 차수 데이터에서 학습된다(이직률 계수 1.0으로 최대).
- **통계법·데이터 변질 — 원천 해소**: 핵심 파이프라인을 승인·로그인이 불필요한 전면 공개 데이터(국민연금 사업장 내역·임금체불 공개 명단·E-9 근무현황)로 재구축했다. MDIS 마이크로데이터는 보조 연구용으로만 남긴다. 산출은 셀 단위로 한정하며 개별 사업장을 지목하지 않는다.
- **남은 한계**: ① 체불 명단은 고액·상습 확정자 위주 선별 표본, ② 국민연금 업종코드(구분류)→현행 대분류 크로스워크는 근사, ③ LNN 조기 경보는 합성 시뮬레이션 단계, ④ 셀 내부 개별 사업장 위험은 구분하지 않음(의도된 설계).
